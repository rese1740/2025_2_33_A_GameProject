...
public class CityGenerator : EditorWindow
{
    ...
    private void GenerateCity()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            ...
            {
                Vector3 pos = new Vector3(x * buildingSpacing, 0, z * buildingSpacing);

                if (x % 2 == 0 || z % 2 == 0)
                {
                    CreateRoad(pos, roadsParent.transform);
                }
                else
                {
                    CreateBuilding(pos, roadsParent.transform);
    ...
    private void CreateBuilding(Vector3 pos, Transform parent)
    {
        GameObject building = GameObject.CreatePrimitive(PrimitiveType.Cube);
        building.name = "Building";

        float height = Random.Range(5.0f, 20.0f);
        building.transform.position = pos + Vector3.up * height / 2.0f;
    ...
    private void CreateRoad(Vector3 pos, Transform parent)
    {
        GameObject road = GameObject.CreatePrimitive(PrimitiveType.Cube);
        road.name = "Building";

        road.transform.position = pos + Vector3.up * 0.1f;
